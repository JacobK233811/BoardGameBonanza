...
@bot.event
async def on_message(message, player1=None, player2=None):
    if message.author == bot.user:
        pass
    else:
        if message.content[:4].lower() == 'sent':
            await message.channel.send("Now specify which stack you want to move.")
        elif message.content[:4].lower() == 'made':
            await message.channel.send("Send a picture followed by the word 'sent'")
        elif message.content[:10].lower() == 'show stacks':
            if player1.discord_id == message.author:
                message.channel.send(f"Stack A: {player1.stack_a}, Stack B: {player1.stack_b}, Stack C: {player1.stack_c}")
            else:
                message.channel.send(f"Stack A: {player2.stack_a}, Stack B: {player2.stack_b}, Stack C: {player2.stack_c}")
        elif message.content[:5].lower() == 'stack':
            stack = message.content.split()[1]
            if player1.discord_id == message.author:
                if stack == 'a':
                    player1.stack_a.move_piece()
                    message.channel.send("Please tell the host your move in row#,col# format. ")
                elif stack == 'b':
                    player1.stack_b.move_piece()
                    message.channel.send("Please tell the host your move in row#,col# format. ")
                elif stack == 'c':
                    player1.stack_c.move_piece()
                    message.channel.send("Please tell the host your move in row#,col# format. ")
                else:
                    message.channel.send("Please tell the host your original piece location in row#,col# format.\nPlease tell the host your final piece location in row#,col# format. ")
        ...
        elif message.content[:4].lower() == "test":
            pass
...
For this Discord bot, players use stack z to designate a move of a piece already on the board. Command !quit will end the game.\n
    ...
    Now choose the host by sending the message 'host [name]'"""
            ...
            message.channel.send("Goodbye! Send the message 'Gobblet' or another game name to start a new experience.")
