...
async def on_message(message):
    if message.author == bot.user:
        ...
        if message.content[:4].lower() == 'sent':
            await message.channel.send("Now specify which stack you want to move with the format 'p# stack (a/b/c/z)'.")
...
2. Type out GoPlay when ready to start playing :blush:"""
        ...
        # elif message.content[:11].lower() == 'show stacks':
        #     if player1.discord_id == message.author:
        #         await message.channel.send(f"Stack A: {player1.stack_a}, Stack B: {player1.stack_b}, Stack C: {player1.stack_c}")
        #     else:
        #         await message.channel.send(f"Stack A: {player2.stack_a}, Stack B: {player2.stack_b}, Stack C: {player2.stack_c}")
    ...
    2. The Player taking their turn will choose the stack they want to play from (send the message "stack a/b/c") or if they want to move an existing piece on the board (send the message "stack z") 
